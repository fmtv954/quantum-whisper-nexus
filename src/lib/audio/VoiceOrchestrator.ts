...
export class VoiceOrchestrator {
  ...
  sendMessage(text: string): void {
    ...
    if (!this.callId) {
      console.warn('[VoiceOrchestrator] Cannot send user text without an active call id');
      return;
    }
  ...
  async disconnect(): Promise<void> {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      const payload: { type: 'end_call'; callId?: string } = { type: 'end_call' };
  ...
  private async startStreamingLocalAudio(): Promise<void> {
    ...
    if (!this.callId) {
      console.warn('[VoiceOrchestrator] Cannot start streaming without an active call id');
      return;
    }
    ...
    this.audioProcessor.onaudioprocess = (event) => {
      ...
      if (!this.callId) {
        console.warn('[VoiceOrchestrator] Cannot send audio chunk without an active call id');
        return;
      }
