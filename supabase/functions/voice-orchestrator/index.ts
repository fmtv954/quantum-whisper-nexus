      ...
      switch (message.type) {
        ...
        case 'audio_chunk': {
          ...
          if (context.deepgramReady && context.deepgramWs && context.deepgramWs.readyState === WebSocket.OPEN) {
            ...
            console.warn(`[WebSocket] Deepgram not ready for call ${currentCallId}, buffering ${audioBytes.byteLength} bytes`);
            context.pendingAudioChunks.push(audioBytes);
          }
          break;
        }
...
async function initializeDeepgramSTT(context: ConversationContext, clientSocket: WebSocket) {
  ...
  deepgramWs.onopen = () => {
    ...
    if (context.pendingAudioChunks.length) {
      ...
      for (const chunk of context.pendingAudioChunks.splice(0)) {
